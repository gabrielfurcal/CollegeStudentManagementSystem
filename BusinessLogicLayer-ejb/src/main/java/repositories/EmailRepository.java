 ...
 */
@Stateless
public class EmailRepository extends BaseRepository<Email> implements IEmailRepository
{
    ...
    public List<Email> findEmails(User user)
    {
        try
        {
            return getEntityManager().createQuery("select e from Email e where e.user = :user", Email.class).setParameter("user", user).getResultList();
        }
        catch(Exception ex)
        {
