...
public class RoleController extends HttpServlet
{
            ...
            throws ServletException, IOException
    {
        try
        {
            ...
            {
                if (!this._permissionRepository.hasUserPermission((int) request.getSession(false).getAttribute("userId"), "/Roles/Create"))
                    throw new Exception("User doesn't have permission");

                doCreateGet(request, response);
            } else if (request.getRequestURI().contains("/Edit"))
            {
                if (!this._permissionRepository.hasUserPermission((int) request.getSession(false).getAttribute("userId"), "/Roles/Edit"))
                    throw new Exception("User doesn't have permission");

                doEditGet(request, response);
            } else if (request.getRequestURI().contains("/Details"))
            {
                if (!this._permissionRepository.hasUserPermission((int) request.getSession(false).getAttribute("userId"), "/Roles/Details"))
                    throw new Exception("User doesn't have permission");

                doDetailsGet(request, response);
            } else if (request.getRequestURI().contains("/Delete"))
            {
                if (!this._permissionRepository.hasUserPermission((int) request.getSession(false).getAttribute("userId"), "/Roles/Delete"))
                    throw new Exception("User doesn't have permission");

                doDeleteGet(request, response);
            } else
            {
                if (!this._permissionRepository.hasUserPermission((int) request.getSession(false).getAttribute("userId"), "/Roles"))
                    throw new Exception("User doesn't have permission");
            ...
            throws ServletException, IOException
    {
        try
        {
            ...
            if (permissionsSelected != null)
            {
                ...
                {
                    Permission permission = this._permissionRepository.findById(Integer.parseInt(permissionSelected));
            ...
            throws ServletException, IOException
    {
        ...
        Role role = this._roleRepository.findById(Integer.parseInt(roleId));
        ...
        else
        {
            ...
            List<Integer> definedPermissionsIds = this._permissionRepository.findPermissions(role).stream().map(Permission::getPermissionId).collect(Collectors.toList());


            request.setAttribute("definedPermissionsIds", definedPermissionsIds);
